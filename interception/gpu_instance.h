...
class GPUInstance {
  public:
    explicit GPUInstance(int device_id, int client_fd)
        : client_fd_(client_fd), vgpu_(fc_shared_mem_path(), (1 << 20) * 9), vgpu_ptr_(vgpu_.get()),
          pinned_memory_(fc_cuda_pin_path(), SHM_SIZE * 5), client_pin_start_(nullptr) {
  ...
  private:
    int client_fd_;
    Response response_;
    void return_result() { send(client_fd_, response_.data(), response_.size(), 0); }
    VirtualGPU vgpu_;
    void *vgpu_ptr_;
